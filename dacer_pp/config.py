...
class DACERppConfig:
    ...
    # ---- 학습 일반 ----
    # 할인율. 유효 지평 = 1/(1-gamma) 스텝 @30Hz.
    # 이력: 0.99(지평 100스텝=3.3s) -> 0.995(200스텝=6.6s, 코너 탈출 속도가 다음
    #   직선에서 회수하는 이득(2~3s 뒤)을 근시안적으로 할인하지 않도록 상향)
    # ★2026-08-17 0.995 -> 0.99 로 되돌림(사용자 지시, 스윕 축).
    #   근거/의도: 랩이 ~44m·약 7s 라 0.995(6.6s)는 사실상 랩 전체를 한 지평에 담아
    #   크리틱이 풀어야 할 크레딧 할당 구간이 너무 길다. 20260812 런에서 Pow 크리틱의
    #   qspread_b 가 0.31~0.40 으로 CVaR(0.72~1.13)의 절반 이하였는데, 이는 Q 가
    #   행동을 구분하지 못한다는 신호이고 조향 다양성 붕괴(pstd 0.013)와 같은 시점이다.
    #   지평을 3.3s 로 줄이면 크리틱 학습이 쉬워져 Q 의 행동 민감도가 회복될 수 있다.
    # ★부작용 주의: 위 상향 근거(근시안적 코너링)가 그대로 되살아날 수 있다.
    #   코너 진입 속도가 낮아지거나 '코너 탈출 가속'을 덜 하는 쪽으로 퇴행하는지
    #   collapse_log 의 r_b(평균 보상)와 랩타임으로 반드시 확인할 것.
    gamma: float = 0.99
    tau: float = 0.005
    actor_lr: float = 1e-4
