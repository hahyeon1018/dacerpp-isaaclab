...
class TireModelCfg:
    ...
    #          mu = 0.99 필요. com_height 를 0.04~0.12 로 흔들어도 0.86~1.08 범위.
    #   => 실 노면 mu ~= 0.8~1.1. (0.75, 1.10) 이 이를 브래킷하고 양쪽에 여유를 둔다.
    ...
    # ---- 실측 코너웨이트 (2026-08-18, generated/weight_{total,front,rear}.jpg) ----
    # 네 바퀴 전체: 앞 2280.0g(50%) / 뒤 2252.0g(50%) / 총 4532.0g
    #   (좌 2202g 49% / 우 2330g 51% — 좌우 불균형은 2D 자전거 모델로는 표현 못 함)
    # 교차검증: 앞바퀴만 2273.0g + 뒷바퀴만 2235.0g = 4508g vs 전체 4532g (오차 0.5%)
    # 이력: 3940.6g(0716, 앞 55/뒤 45) -> 4240 추정(0812) -> 4532 실측(0818)
    #   0812 의 '증가분이 어디에 붙었나' 미지수는 이제 풀렸다: 뒤축이다.
    #   그 주석이 예측한 '전부 뒤축' 값(lf=0.162, lr=0.168)과 실측(0.164/0.166)이 거의 같다.
    # ★ assets/f1tenth/f1tenth.urdf 의 base_link mass 를 4.032 로 함께 바꿨다
    #   (base_link 4.032 + 조향너클 2x0.05 + 바퀴 4x0.1 = 4.532). USD 재생성 필요 —
    #   재생성 전에는 PhysX 가 여전히 옛 질량으로 적분하므로 '4.53kg 의 그립으로
    #   4.24kg 를 가속'하는 비물리가 남는다. 절차는 README 의 convert_urdf.py 참조.
    mass: float = 4.532           # 하중 계산용(kg) — 운동 적분은 PhysX(USD) 질량 사용
    com_height: float = 0.07       # 무게중심 높이(m): 종방향 하중이동 (★여전히 미실측)
    # 앞뒤 배분 실측 -> lf = L x 뒤축하중비, lr = L x 앞축하중비 (CoM 모멘트 균형)
    #   앞축하중비 2280/4532 = 0.5031, 뒤축하중비 2252/4532 = 0.4969
    # CoM 이 축거 중앙에 거의 정확히 왔다(구 55:45 -> 50:50). 뒤로 15mm 이동했으므로
    # 구 값보다 오버스티어 경향이 커진다 — k_slip / slip_deadzone 재확인 대상.
    lf: float = 0.164              # CoM -> 앞축 거리(m) = 0.33 x 0.4969
    lr: float = 0.166              # CoM -> 뒤축 거리(m) = 0.33 x 0.5031
    ...
    #   F = m*a + 구름저항(c_roll*m*g = 0.67N) = 6.08*4.532 + 0.67 = 28.2N
    ...
    #   뒤축 마찰서클(mu*N_r)이다. 위 모델을 그대로 풀어 본 결과(mass 4.532/lf 0.164/h 0.07):
    #       mu    가속        제한요인        제동        제한요인
    #       0.45  2.43 m/s^2  마찰서클       -2.00 m/s^2  마찰서클
    #       0.60  3.35        마찰서클       -2.59        마찰서클
    #       0.75  4.35        마찰서클       -3.09        f_brake_max
    #       0.93  5.61        마찰서클       -3.09        f_brake_max
    #       1.05  6.18        f_drive_max    -3.09        f_brake_max
    #   즉 f_drive_max 를 22->28 로 올린 효과는 mu >= 1.0 에서만 나타난다.
    #   (검증: 구 파라미터 mass 4.24/lf 0.149/22N/mu 1.05 로 이 모델을 풀면 직선제동
    #    -3.80 m/s^2 인데, 시뮬 실측 문서 PART 1-B 의 -3.86 과 일치한다 -> 모델 신뢰 가능.)
    f_drive_max: float = 28.0      # 구동력 상한(N) ≈ 6.18 m/s^2 @4.532kg (구 22.0)
    ...
    #   않으므로 구동만큼 세게 못 세운다.
    #   구 코드는 clamp(±f_drive_max) 로 대칭이었다. f_drive_max 를 28 로 올리면서
    #   대칭을 유지하면 시뮬 제동이 6.18 m/s^2 = 실차의 2배가 되어 '학습에서는 늦게
    #   밟아도 서는데 실차에서는 못 서는' 최악의 방향으로 어긋난다.
    f_brake_max: float = 14.0      # 제동력 상한(N) ≈ 3.09 m/s^2 @4.532kg
